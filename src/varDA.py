...
def main():
    ...
    if ( varDA.precondition ):
        U,S2,UT = np.linalg.svd(Bc, full_matrices=True, compute_uv=True)
    ...
    for k in range(0, DA.nassim):
        ...
        # update step
        if ( fdvar ): xa, niters = update_varDA(xb, Bc, ywin, R, H, varDA, model=model)
        else:         xa, niters = update_varDA(xb, Bc, y,    R, H, varDA)
