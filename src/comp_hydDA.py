...
def main():
    ...
    # name of starting hybrid output diagnostic file, starting index and no.
    # of files
    ...
    for fname in fnames:
        ...
        xt, Xb, Xa, y, _, _, xbc, xac, _, _ = read_diag(
    ...
    if save_figures:
        fig.savefig(
            '%s_%shybDA_RMSE_%sVar_Prior.eps' %
            (model.Name,
             vstr,
             vstr),
            dpi=300,
    ...
    if save_figures:
        fig.savefig(
            '%s_%shybDA_RMSE_%s_Posterior.eps' %
            (model.Name,
             vstr,
             estr),
            dpi=300,
    ...
    if save_figures:
        fig.savefig(
            '%s_%shybDA_RMSE_%sVar_Posterior.eps' %
            (model.Name, vstr, vstr), dpi=300, orientation=fOrient, format='eps')
    # -----------------------------------------------------------
