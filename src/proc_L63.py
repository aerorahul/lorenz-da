...
###############################################################
# proc_L63.py - process the diagnostics written by L63_???DA.py
###############################################################
...
def main():
    ...
    fig = plot_trace(obs=y, ver=xt, xb=xbm, xa=xam, N=ndof)
    ...
    if ( do_hybrid ):
        ...
        xyrmse = np.sqrt( np.sum( (xt - y)**2 ) / ndof )
        fig = plot_trace(obs=y, ver=xt, xb=xbc, xa=xac, N=ndof)
