...
def main():
    ...
    for k in range(DA.nassim):
        ...
        for outer in range(DA.maxouter):
            ...
            # blend covariance from flow-dependent (ensemble) and static
            # (climatology)
            B = (1.0 - DA.hybrid_wght) * Bs + DA.hybrid_wght * (Be * L)
            if (varDA.precondition == 1):
                [U, S2, _] = np.linalg.svd(
                    B, full_matrices=True, compute_uv=True)
        ...
        if (varDA.update == 2):
            ...
            for i in range(varDA.fdvar.nobstimes):
                ...
                Xb = advance_ensemble(
                    Xa, varDA.fdvar.twind_obs, model, perfect=False)
                Xa, evratio = update_ensDA(Xb, y[i, :], R, H, ensDA, model)

                if (varDA.fdvar.tb +
                        varDA.fdvar.twind_obsIndex[i] > varDA.fdvar.ta):
                    break
