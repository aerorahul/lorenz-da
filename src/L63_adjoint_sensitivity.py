...
# plot x v/s idim plots ( eg. 1 gives x-y plot, 2 gives x-z plot [most common] )
idim = 2
...
sxi_tlm = integrate.odeint(L63_tlm, sxi, tint, (par,xsave,tsave,False),rtol=tol,atol=tol)
z = sxi_tlm[-1,:].copy()
zTz = np.dot(np.transpose(z),z)

axs = integrate.odeint(L63_tlm, z, tint, (par,np.flipud(xsave),tsave,True),rtol=tol,atol=tol)
z0 = axs[-1,:].copy()
...
pyplot.plot(np.array([xsave[0,0],  xsave[0,0]  + vs * sxi[0]]), np.array([xsave[0,idim],  xsave[0,idim]  + vs * sxi[idim]]), 'g-',linewidth=4)

# forecast time sensitivity gradient vector
pyplot.plot(np.array([xsave[-1,0], xsave[-1,0] + vs * sxf[0]]), np.array([xsave[-1,idim], xsave[-1,idim] + vs * sxf[idim]]), 'g-',linewidth=2)

pyplot.xlabel('X',fontweight='bold',fontsize=12)
pyplot.ylabel('modeled Z',fontweight='bold',fontsize=12)
pyplot.title('Lorenz attractor, Control Trajectory, and Sensitivity Gradient Vector',fontweight='bold',fontsize=14)
...
for k in range(0,np.shape(px)[0],4):
    pyplot.plot(np.array([xsave[k,0],  xsave[k,0]  + vs * px[k,0]]), np.array([xsave[k,idim],  xsave[k,idim]  + vs * px[k,idim]]), 'b-',linewidth=2)
...
pyplot.title('Lorenz 1963 Adjoint Sensitivity Check as a Function of IC Amplitude',fontweight='bold',fontsize=14)
