...
def main():
    ...
    parser = ArgumentParser(
        description='Process the diag file written by ???DA.py',
        formatter_class=ArgumentDefaultsHelpFormatter)
    parser.add_argument(
        '-f',
        '--filename',
        help='name of the diag file to read',
        type=str,
        required=True)
    parser.add_argument(
        '-m',
        '--measure',
        help='measure to evaluate performance',
        type=str,
        required=False,
        choices=[
            'obs',
    ...
    # get model, DA class data and necessary attributes from the diagnostic
    # file:
    ...
    if (DA.do_hybrid):
        xt, Xb, Xa, y, H, R, xbc, xac, niters, evratio = read_diag(
    ...
    if ((DA.do_hybrid) or (hasattr(varDA, 'update'))):
        ...
        for i in range(DA.nassim):
            if (DA.do_hybrid):
                ...
                xtmp = xbm
            innov = np.nansum((y[i, :model.Ndof] -
                               np.dot(np.diag(H[i, :]), xtmp[i, :]))**2)
            if (DA.do_hybrid):
                Bc = (1.0 - DA.hybrid_wght) * Bs + \
                    DA.hybrid_wght * (np.cov(Xb[i,], ddof=1) * L)
            else:
                Bc = Bs
    ...
    if (DA.do_hybrid):
        ...
        if (hasattr(varDA, 'update')):
            fstr, niters = vstr, tmpvar
        elif (hasattr(ensDA, 'update')):
            fstr, evratio = estr, tmpvar
    ...
    if (measure == 'truth'):
        ...
        xbrmse = np.sqrt(
            np.sum((y[:, :model.Ndof] - xbm)**2, axis=1) / model.Ndof)
        xarmse = np.sqrt(
            np.sum((y[:, :model.Ndof] - xam)**2, axis=1) / model.Ndof)
    ...
    fig = plot_rmse(
        ...
        pretitle=fstr)
    if (save_fig):
        save_figure(fig, fname=fname_fig + '%s-RMSE' % fstr)
    ...
    if (model.Name == 'L63'):
        fig = plot_trace(
            ...
            pretitle=fstr)
        if (save_fig):
            save_figure(fig, fname=fname_fig + 'attractor')

    elif (model.Name == 'L96'):
        fig = plot_L96(obs=y[pIndex, :model.Ndof], ver=xt[pIndex,], xb=Xb[pIndex,],
                       xa=Xa[pIndex,], t=pIndex + 1, N=model.Ndof, pretitle=fstr)
        if (save_fig):
            save_figure(fig, fname=fname_fig + '%s-%d' % (fstr, pIndex + 1))
    ...
    if (DA.do_hybrid):
        if (measure == 'truth'):
            ...
            xbrmse = np.sqrt(
                np.sum((y[:, :model.Ndof] - xbc)**2, axis=1) / model.Ndof)
            xarmse = np.sqrt(
                np.sum((y[:, :model.Ndof] - xac)**2, axis=1) / model.Ndof)
        xyrmse = np.sqrt(np.sum((y[:, :model.Ndof] - xt)**2) / model.Ndof)
        fig = plot_rmse(
            xbrmse=xbrmse,
            xarmse=xarmse,
            ...
            pretitle=vstr)
        if (save_fig):
            save_figure(fig, fname=fname_fig + '%s-RMSE' % vstr)
        ...
        if (model.Name == 'L63'):
            ...
            if (save_fig):
                save_figure(
                    fig, fname=fname_fig + '%s-%d' %
                    (vstr, pIndex + 1))

    # plot the iteration statistics and/or error-to-variance ratio
    if (hasattr(varDA, 'update')):
        for outer in range(DA.maxouter):
            fig = plot_iteration_stats(
                np.squeeze(niters[:, outer]), pretitle=vstr)
            if (save_fig):
                save_figure(
                    fig, fname=fname_fig + '%s-niters-%d' %
                    (vstr, outer + 1))
        fig = plot_error_variance_stats(
            var_evratio, sStat=sStat, pretitle=vstr)
        if (save_fig):
            save_figure(fig, fname=fname_fig + '%s-evratio' % vstr)
    if (hasattr(ensDA, 'update')):
        titlestr = estr + ' - Innovation Variance / Total Variance'
        fig = plot_error_variance_stats(evratio, sStat=sStat, pretitle=estr)
        if (save_fig):
            save_figure(fig, fname=fname_fig + '%s-evratio' % estr)
