...
def main():
    ...
    for k in range(sOI, eOI):
        ...
        B = np.cov(Xbi, ddof=1)
        if (DA.do_hybrid):
            Bs = (1.0 - DA.hybrid_wght) * Bc + DA.hybrid_wght * B
        ...
        # metric : J = (x^T)Wx ; dJ/dx = J_x = 2Wx ; choose W = I, x = xfmet,
        # J_x = Jxf
        ...
        JbHXb = np.dot(Jbp, np.transpose(
            np.dot(H[valInd, :], Xbpi))) / (ensDA.Nens - 1)
        Kmb = np.linalg.inv(
            np.cov(np.dot(H[valInd, :], Xbi), ddof=1) + np.diag(R[valInd, valInd]))

        xfmet = np.transpose(mxf * (np.transpose(Xaf) - xtf))
        Ja = 0.5 * np.diag(np.dot(np.transpose(xfmet), xfmet))
        Jap = Ja - np.mean(Ja, axis=0)
        JaHXa = np.dot(Jap, np.transpose(
            np.dot(H[valInd, :], Xapi))) / (ensDA.Nens - 1)
        ...
        print('dJe = %12.5f | dJe_a = %12.5f | dJe_b = %12.5f ' %
              (np.nansum(e_dJa + e_dJb), np.nansum(e_dJa), np.nansum(e_dJb)))

        # A D J O I N T - based observation impact

        if (DA.do_hybrid):
            K = np.dot(Bs, np.dot(np.transpose(H[valInd, :]), np.linalg.inv(np.dot(
                H[valInd, :], np.dot(Bs, np.transpose(H[valInd, :]))) + np.diag(R[valInd, valInd]))))
        else:
            K = np.dot(B, np.dot(np.transpose(H[valInd, :]), np.linalg.inv(
                np.cov(np.dot(H[valInd, :], Xbi), ddof=1) + np.diag(R[valInd, valInd]))))

        Jxbf = mxf * (xbmf[-1, :] - xtf)
        Jxb = advance_model_tlm(
            model,
            Jxbf,
            tf,
            xbmf,
        ...
        print('dJa = %12.5f | dJa_a = %12.5f | dJa_b = %12.5f ' %
              (np.nansum(a_dJa + a_dJb), np.nansum(a_dJa), np.nansum(a_dJb)))
