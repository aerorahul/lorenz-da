...
def main():
    ...
    if (DA.do_hybrid):
        ...
        [_, Xb, _, _, _, _, _] = read_diag(fdiag, 0)
    ...
    for i in range(0, model.Ndof):
        for j in range(0, model.Ndof):
            ...
            print('i = %2d, j = %2d, d = %5.3f, c = %10.8f, c = %10.8f' %
                  (i, j, dist, L[i, j], L2[i, j]))
    ...
    for i in range(0, ensDA.Nens):
        ...
        XbpLb[:, start:end] = np.dot(
            np.diag(Xbp[:, i]), L2) / np.sqrt(ensDA.Nens - 1)
        XbpLl[:,
              start:end] = L2 * np.repeat(np.transpose(np.matrix(Xbp[:,
                                                                     i])),
                                          model.Ndof,
                                          axis=1) / np.sqrt(ensDA.Nens - 1)
    ...
    fig3 = plot_cov(
        Bs * L,
        title="Static Schur : $\\mathbf{B}_s \\circ\\ \\mathbf{L}$")
    fig4 = plot_cov(
        Be * L,
        title="Ensemble Schur : $\\mathbf{B}_e \\circ\\ \\mathbf{L}$")
    fig5 = plot_cov(
        Be_Lb,
        title="Ensemble Buehner : $[\\mathbf{X}^'_b \\mathbf{L}] [\\mathbf{X}^'_b \\mathbf{L}]^{T}$")
    ...
    fig11 = plot_cov(
        Be * L - Be_Lb,
        title="Difference Ensemble Schur - Ensemble Buehner")
    fig12 = plot_cov(
        Be * L - Be_Ll,
        title="Difference Ensemble Schur - Ensemble Liu")
    fig13 = plot_cov(
        Be_Lb - Be_Ll,
        title="Difference Ensemble Buehner - Ensemble Liu")
