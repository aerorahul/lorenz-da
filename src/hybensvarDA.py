    ...
    for k in range(DA.nassim):
        ...
        for outer in range(DA.maxouter):
            ...
            for i in range(1, varDA.fdvar.nobstimes):
                Xbwin[i, :, :] = advance_ensemble(
                    Xbwin[i - 1, :, :], varDA.fdvar.twind_obs, model, perfect=False)

            # compute static background error cov.
            Bs = compute_B(varDA, Bc, outer=outer)

            # construct block diagonal from static background error cov. and
            # localization matrices
            B = linalg.block_diag(
                (1.0 - DA.hybrid_wght) * Bs,
                DA.hybrid_wght * A)
            ...
            # update step
            xac, niters = update_hybensvarDA(
                xbc, B, C, np.squeeze(y), R, H, varDA, model)

            # write diagnostics to disk for each outer loop (at the beginning
            # of the window)
            write_diag(
                diag_file.filename,
                k + 1,
        ...
        if (DA.hybrid_rcnt):
            Xa = (Xa.T - np.mean(Xa, axis=1) + xac).T

        # if doing 4Dvar, step to the next assimilation time from the beginning
        # of assimilation window
        if (varDA.update == 2):
            xt = model.advance(xt, varDA.fdvar.tanal,
                               perfect=True)[-1, :].copy()
            xac = model.advance(xac, varDA.fdvar.tanal,
                                perfect=False)[-1, :].copy()
