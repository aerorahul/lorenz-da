...
def plot_abs_error(xbe, xae, label=['x'], N=1, yscale='semilog', figNum=None):
    ...
    if ((yscale != 'linear') and (yscale != 'semilog')):
        yscale = 'semilog'
    ...
    for k in range(0, N):
        pyplot.subplot(N, 1, k + 1)
        if (yscale == 'linear'):
            pyplot.plot(np.abs(xbe[k, :]), 'b-',
                        label='background', linewidth=2)
            pyplot.plot(np.abs(xae[k, :]), 'r-', label='analysis', linewidth=2)
        elif (yscale == 'semilog'):
            pyplot.semilogy(np.abs(xbe[k, :]), 'b-',
                            label='background', linewidth=2)
            pyplot.semilogy(np.abs(xae[k, :]), 'r-',
                            label='analysis', linewidth=2)
        pyplot.ylabel(label[k], fontweight='bold', fontsize=12)
        strb = 'mean background : %5.4f +/- %5.4f' % (
            np.mean(np.abs(xbe[k, 1:])), np.std(np.abs(xbe[k, 1:]), ddof=1))
        stra = 'mean analysis : %5.4f +/- %5.4f' % (
            np.mean(np.abs(xae[k, 1:])), np.std(np.abs(xae[k, 1:]), ddof=1))
...
def plot_rmse(
    ...
    if (yscale == 'linear'):
        if (xbrmse is not None):
            pyplot.plot(xbrmse[1:], 'b-', label='prior',
        ...
        if (xbrmse is not None):
            pyplot.semilogy(xbrmse[1:], 'b-',
                            label='prior', linewidth=2, alpha=0.90)
        if (xarmse is not None):
    ...
    if (xbrmse is not None):
        strb = 'mean prior rmse : %5.4f +/- %5.4f' % (
            np.mean(xbrmse[sStat + 1:]), np.std(xbrmse[sStat + 1:], ddof=1))
        pyplot.text(0.05 * len(xbrmse), yl[1] - 0.1 * dyl, strb, fontsize=10)
    if (xarmse is not None):
        stra = 'mean posterior rmse : %5.4f +/- %5.4f' % (
            np.mean(xarmse[sStat:-1]), np.std(xarmse[sStat:-1], ddof=1))
        pyplot.text(0.05 * len(xarmse), yl[1] - 0.15 * dyl, stra, fontsize=10)
    if (xyrmse is not None):
        stro = 'mean observation rmse : %5.4f +/- %5.4f' % (
            np.mean(xyrmse[sStat:]), np.std(xyrmse[sStat:], ddof=1))
...
def plot_abs_error_var(
    ...
    for k in range(0, N):
        ...
        pyplot.ylabel(label[k], fontweight='bold', fontsize=12)
        strb = 'mean background : %5.4f +/- %5.4f' % (
            np.mean(xbev[k, 2:]), np.std(xbev[k, 2:], ddof=1))
        stra = 'mean analysis : %5.4f +/- %5.4f' % (
            np.mean(xaev[k, 2:]), np.std(xaev[k, 2:], ddof=1))
        yl = pyplot.get(pyplot.gca(), 'ylim')
        if (yscale == 'linear'):
            yoffb = 0.9 * yl[1]
            yoffa = 0.8 * yl[1]
...
def plot_error_variance_stats(evratio, figNum=None, sStat=100, pretitle=None):
    ...
    pyplot.plot(np.ones(len(evratio) - 1) *
                np.mean(evratio[sStat:]), 'g-', linewidth=1)

    pyplot.ylim(0.0, 3.0)
    str = 'mean E/V  : %5.4f +/- %5.4f' % (
        np.mean(evratio[sStat:]), np.std(evratio[sStat:], ddof=1))
    pyplot.text(0.05 * len(evratio), 0.2, str, fontsize=10)

    pyplot.xlabel('Assimilation Step', fontweight='bold', fontsize=12)
    pyplot.ylabel(
        'Innovation Variance / Total Variance',
        fontweight='bold',
        fontsize=12)
...
def plot_ObImpact(
    ...
    stra = r'mean $\delta J_a$ : %5.4f +/- %5.4f' % (
        np.mean(dJa), np.std(dJa, ddof=1))
    stre = r'mean $\delta J_e$ : %5.4f +/- %5.4f' % (
        np.mean(dJe), np.std(dJe, ddof=1))
